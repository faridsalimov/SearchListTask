...
class Program
{
    public static List<Debtor> debtors = new List<Debtor> {
        ...
        new Debtor("Richard M. Thomasasdsadsa", DateTime.Parse("February 13, 1972"), "510-735-33597777", "RichardMThomas@jourrapide.com", "4454 Green Avenue Oakland, CA 94609", 7875),
        ...
        new Debtor("Pamela H. Beauchamp", DateTime.Parse("December 31, 1959"), "801-559-6347", "PamelaHBeauchamp@jourrapide.com", "3239 Tori Lane Salt Lake City, UT 84104", 6588)
    };
    ...
    static void Main()
    {
        ...
        //var result1 = debtors.Where(d => d.Email.EndsWith("rhyta.com") || d.Email.EndsWith("dayrep.com")).ToList();
        //result1.ForEach(d => Console.WriteLine(d));

        //2) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag

        //var result2 = debtors.Where(d => 26 < DateTime.Now.Year - d.BirthDay.DayOfYear && DateTime.Now.Year - d.BirthDay.Year < 36).ToList();
        //result2.ForEach(d => Console.WriteLine(d));

        //3) Borcu 5000 - den cox olmayan borclularic cixartmag

        //var result3 = debtors.Where(d => d.Debt < 5000).ToList();
        //result3.ForEach(d => Console.WriteLine(d));

        //4) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq

        //var result4 = debtors.Where(d => d.FullName.Length > 18 && d.Phone.Count(c => c == '7') >= 2).ToList();
        //result4.ForEach(d => Console.WriteLine(d));

        //5) Qishda anadan olan borclulari cixardmaq

        //var result5 = debtors.Where(d => d.BirthDay.Month("December") || d.BirthDay.Month("January") || d.BirthDay.Month("February")).ToList();
        //result5.ForEach(d => Console.WriteLine(d));
        ...
        //var result7 = debtors.Where(d => pre(d)).Select(d => d.FullName + " " + (DateTime.Now.Year - d.BirthDay.Year) + " " + d.Debt).ToList();
        //result7.ForEach(d => Console.WriteLine(d));
        ...
        //var result9 = debtors.Select(d => d.BirthDay.Year).ToList();
        //var resultt = result9.GroupBy(i => i).OrderByDescending(g => g.Count()).First().Key;
        //Console.WriteLine(resultt);

        //10)Borcu en boyuk olan 5 borclunun siyahisini cixartmaq

        //var result10 = debtors.Select(d => d).ToList();
        ...
        //var result11 = debtors.Select(d => d.Debt).Sum();
        //Console.WriteLine($"Total: {result11}$");

        //12)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq

        //var result12 = debtors.Where(d => d.BirthDay.Year <= 1941).ToList();
        //result12.ForEach(d => Console.WriteLine(d));
        ...
        //var result14 = debtors.Where(d => (500 * (d.BirthDay.Month - DateTime.Now.Month)) == d.Debt).ToList();
        //result14.ForEach(d => Console.WriteLine(d.FullName + ": " + d.BirthDay));

        //15)Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq

        //var result15 = debtors.Where(d => d.FullName.Contains("s") && d.FullName.Contains("m") && d.FullName.Contains("i") && d.FullName.Contains("l") && d.FullName.Contains("e")).ToList();
        //result15.ForEach(d => Console.WriteLine(d.FullName));
    }
}